...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    try:
        cut = raw.find(b"\r\n\r\n")
        if cut == -1:
            return None, "", "Invalid response: invalid separator"
        
        header = raw[:cut]
        body = raw[cut + 4:]

        header_line = header.split(b"\r\n")
        header_line_split = header_line[0].split(b" ")
        status_code = int(header_line_split[1])

        preview_byte = body[:max_preview]
        preview = preview_byte.decode("utf-8")
