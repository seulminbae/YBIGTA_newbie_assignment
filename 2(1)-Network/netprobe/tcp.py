...
def connect_with_fallback(ips: list[str], port: int, timeout: float, prefer: str = "any") -> TCPConnectResult:
    ...
    for ip in ordered:
        result = connect_one(ip, port, timeout)
        try:
            if result[0] is not None:
                sock, connect_ms = result[0], result[1]

                local_addr = result[0].getsockname()
                peer_addr = result[0].getpeername()
            ...
            last_err = f"Socket address get failure: {e}"
            sock.close()
